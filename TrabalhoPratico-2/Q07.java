...
public class Q07 extends Jogador {
    ...
    public static void ordenar(Jogador vetor[], int n){
        for(int i=1;i<n;i++){
            Jogador tmp = vetor[i];
            int j = i - 1; 
            while(j>-1 && (tmp.getAnoNascimento() < vetor[j].getAnoNascimento() || (tmp.getAnoNascimento() == vetor[j].getAnoNascimento() && tmp.getNome().compareTo(vetor[j].getNome()) < 0))){
                if(tmp.getAnoNascimento() == vetor[j].getAnoNascimento())
                    comp+=3;
                else 
                    comp++;    
                vetor[j+1] = vetor[j];
