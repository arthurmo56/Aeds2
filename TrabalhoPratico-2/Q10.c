...
void quicksort(Jogador vetor[], int dir, int esq){
    int pivo = esq;
    int j=dir,i=esq;
    while(i<=j){
        while(comparar1(vetor[i].estadoNascimento,vetor[pivo].estadoNascimento)==1 || 
              (comparar1(vetor[i].estadoNascimento,vetor[pivo].estadoNascimento)==0 && comparar1(vetor[i].nome,vetor[pivo].nome)==1)){
            i++;
            comp++;
        }
        while(comparar1(vetor[j].estadoNascimento,vetor[pivo].estadoNascimento)==2 || 
              (comparar1(vetor[j].estadoNascimento,vetor[pivo].estadoNascimento)==0 && comparar1(vetor[j].nome,vetor[pivo].nome)==2)){
            j--;
            comp++;
        }
        if(i<=j){
    ...
    if(esq<j)
        quicksort(vetor,j, esq);
    if(i<dir)
        quicksort(vetor,dir, i);
...
int main()
{
    ...
    FILE *arquivo;
    arquivo = fopen("matrícula_quicksort.txt","w");
