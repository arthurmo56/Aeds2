...
void subinsercao(Jogador vetor[], int cor, int h, int n)
{
    for(int i=(h+cor); i<n; i+=h)
    {
        Jogador tmp = vetor[i];
        int j = i - h;
        while ((j >= 0) && ((tmp.peso < vetor[j].peso) ||
               ((tmp.peso == vetor[j].peso) && (strcmp(tmp.nome, vetor[j].nome) < 0))))
        {
            comp+=2;
            vetor[j+h] = vetor[j];
            mov++;
...
void Shellsort(Jogador vetor[], int n)
{
    int h=1;
    do
    {
...
int main()
{
    ...
    FILE *arquivo;
    arquivo = fopen("matrícula_shellsort.txt","w");
